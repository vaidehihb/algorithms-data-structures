...
public class MaxHeap<T extends Comparable<T>> {
    ...
    public void increseKey(int k, T val){
        if(k <= size){
            if(val.compareTo(heap[k]) < 0) throw new RuntimeException("Key smaller than original");
            heap[k] = val;
            perlocateUp(k);
        }
    }
    ...
    public void print(){
        for (int i = 1; i <= size / 2; i++) {
            if(hasParent(i)) System.out.print(" PARENT : " + heap[i]);
